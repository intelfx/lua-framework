...
function util.assertfmt(expr, fmt, ...)
	assert(expr, string.format(fmt, ...))
end

assertfmt = util.assertfmt
...
local function __map_one(input, op)
end

function util.map(input, op)
	local result = { }
...
function util.map_tuple(op, i, ...)
	if util.is_function(op) then
		if select('#', ...) > 0 then
			return op(i), util.map_tuple(op, ...)
		else
			return op(i)
		end
	else -- assume op to be field name to extract
		assert(util.is_table(i), "We're told to extract field '%s' from each argument, but next argument is a %s", tostring(op), type(i))

		if select('#', ...) > 0 then
			return i[op], util.map_tuple(op, ...)
		else
			return i[op]
		end
	end
...
function util.tonumber_or_nil(arg)
	if arg then
